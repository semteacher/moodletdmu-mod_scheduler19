<?PHP  // $Id: lib.php,v 1.13.10.11 2011/03/15 21:46:17 diml Exp $
...
function scheduler_cron () {
    ...
    if ($slots){
        foreach ($slots as $slot) {
            ...
            // get course
            $course = get_record('course', 'id', $slot->course);
            ...
            if ($appointments){
                foreach($appointments as $appointed){
                    $student = get_record('user', 'id', $appointed->studentid);
                    $vars = scheduler_get_mail_variables (NULL, $slot, $teacher, $student);
            ...
            foreach($finals as $student => $aGradeSet){
                switch ($scheduler->gradingstrategy){
                    ...
                    case SCHEDULER_MEAN_GRADE:
                        $finalgrades[$student] = ($aGradeSet->count) ? $aGradeSet->sum / $aGradeSet->count : 0 ;
                        break;
                }
            }
